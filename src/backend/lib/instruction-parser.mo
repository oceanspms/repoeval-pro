...
module {
  ...
  public func parseInstructions(notesText : Text) : Types.WeightOverrides {
    ...
    // Detect each keyword condition (independent checks — later ones may add to list)
    ...
    let demo_mult         : Float = 1.0;
    var ai_mult           : Float = 1.0;
    var ignore_notes      : Bool  = false;
    var instructions      : [Text] = [];

    // ── ignore notes ─────────────────────────────────────────────────────────
    if (lower.contains(#text "ignore prompt log") or lower.contains(#text "ignore notes")) {
      ignore_notes := true;
      instructions := instructions.concat(["ignore_notes"]);
    };

    // ── docker / dockerfile weighting ────────────────────────────────────────
    if (lower.contains(#text "weight dockerfile") or
        lower.contains(#text "prioritize docker") or
        lower.contains(#text "focus on docker")) {
      completeness_mult := Float.max(completeness_mult, 2.0);
      docs_mult := Float.max(docs_mult, 1.5);
      instructions := instructions.concat(["weight_dockerfile"]);
    };
    ...
    if (lower.contains(#text "weight docs") or lower.contains(#text "prioritize documentation")) {
    ...
    // ── AI usage skip ─────────────────────────────────────────────────────────
    if (lower.contains(#text "skip ai") or lower.contains(#text "ignore ai")) {
      ai_mult := 0.0;
      instructions := instructions.concat(["skip_ai"]);
    };
